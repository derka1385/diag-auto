...
class DiagnosticContextBuilder:
    ...
    def build(self,db:Session,case:DiagnosticSession)->tuple[dict,list[DiagnosticImage]]:
        ...
        for code in codes:
            ...
            definitions.append({"code":code,"description":row.generic_description if row else None,"source_id":row.source_id if row else None,"manufacturer_specific":manufacturer_specific,"definition_reliability":("indicative_manufacturer" if manufacturer_specific else "generic_standard") if row else "unknown"})
