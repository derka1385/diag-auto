...
def _mock_analysis(context):
    ...
    return DiagnosticAnalysis.model_validate({"schemaVersion":"1.0","caseSummary":"Synthèse de démonstration fondée uniquement sur le dossier fourni.","interpretedFaultCodes":interpreted,"correlations":correlations,"hypotheses":hypotheses,"imageEvidence":[{"imageId":x["id"],"observation":"Image reçue ; le fournisseur mock n’effectue pas d’interprétation visuelle.","confidence":0,"limitations":["Analyse visuelle Gemini désactivée"]} for x in image_rows],"urgency":{"level":"medium","explanation":"Informations insuffisantes pour exclure un risque.","drivingRecommendation":"insufficient_information"},"missingInformation":[{"field":"mesures OBD","reason":"Nécessaires pour discriminer les causes.","howToObtain":"Effectuer le premier contrôle proposé.","priority":"high"}],"nextChecks":[check],"finalConclusion":{"status":"testing_required","summary":"Des contrôles sont requis avant toute conclusion."},"warnings":["Analyse mock clairement identifiée.","Ne remplacer aucune pièce sur cette seule synthèse."]})
